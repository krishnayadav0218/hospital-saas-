...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const allAppointmentsForPatient = ((apptRows && apptRows.value) || [])
    ...
    const allInvoicesForPatient = ((invRows && invRows.value) || [])
    ...
    const allVitalsForPatient = ((trackRows && trackRows.value) || [])
      .filter((v) => v.patientId === match.id)
      .sort((a, b) => String(b.date).localeCompare(String(a.date)));

    // Full-history stats for the dashboard cards — computed before the
    // lists below get truncated to a manageable "recent" slice for display.
    const todayStr = new Date().toISOString().slice(0, 10);
    const upcoming = allAppointmentsForPatient
    ...
    const stats = {
      totalAppointments: allAppointmentsForPatient.length,
      upcoming: upcoming ? { date: upcoming.date, time: upcoming.time, doctorName: doctorName(upcoming.doctorId) } : null,
      pendingDue: allInvoicesForPatient.reduce((s, i) => s + i.balance, 0),
      lastVitalsDate: allVitalsForPatient.length ? allVitalsForPatient[0].date : null,
    };

    const appointments = allAppointmentsForPatient
      .slice(0, 20)
      .map((a) => ({
        id: a.id, date: a.date, time: a.time, reason: a.reason || '', status: a.status,
        doctorName: doctorName(a.doctorId),
        diagnosis: a.diagnosis || '',
        medicines: Array.isArray(a.medicines) ? a.medicines : [],
      }));

    const vitals = allVitalsForPatient
      .slice(0, 10)
      .map((v) => ({ date: v.date, sys: v.sys, dia: v.dia, pulse: v.pulse, temp: v.temp, spo2: v.spo2 }));

    const invoices = allInvoicesForPatient.slice(0, 20);

...
create table if not exists public.profiles (
  ...
  hospital_ids uuid[] not null default '{}',       -- which hospitals this user can access; empty = all (backward-compatible default — admins are always unrestricted regardless)
  created_at   timestamptz not null default now()
);

alter table public.profiles enable row level security;
alter table public.profiles add column if not exists hospital_ids uuid[] not null default '{}';
...
-- Hospital-scoped access: admins can always access every hospital. Any
-- other active user can access a hospital if either (a) their
-- hospital_ids list is empty — the default, meaning "not yet
-- restricted, sees everything" so nothing breaks for existing accounts
-- — or (b) that hospital's id is explicitly in their hospital_ids list.
-- An admin sets hospital_ids from the Users page to actually restrict
-- someone to specific hospital(s).
create or replace function public.can_access_hospital(hid uuid)
returns boolean
language sql security definer set search_path = public stable
as $$
  select case
    when public.is_admin() then true
      ...
      (select cardinality(hospital_ids) = 0 or hid = any(hospital_ids)
...
create policy "active users read hospitals" on public.hospitals
  for select using (public.is_active_user() and public.can_access_hospital(id));
...
create policy "category read" on public.hospital_data
  for select using (public.category_read_ok(category) and public.can_access_hospital(hospital_id));

create policy "category insert" on public.hospital_data
  for insert with check (public.category_write_ok(category) and public.can_access_hospital(hospital_id));

create policy "category update" on public.hospital_data
  for update using (public.category_write_ok(category) and public.can_access_hospital(hospital_id))
  with check (public.category_write_ok(category) and public.can_access_hospital(hospital_id));

create policy "category delete" on public.hospital_data
  for delete using (public.is_admin());

-- ─────────────────────────── patient documents ───────────────────────────
-- Private Storage bucket for lab reports / prescriptions / scans attached
-- to a patient. Files live at {hospital_id}/{patient_id}/{filename}; only
-- the path + small metadata (name, size, uploader) is stored on the
-- patient's own JSON record in hospital_data — no new table needed for
-- that part. These policies control the *files themselves*.
insert into storage.buckets (id, name, public)
values ('patient-documents', 'patient-documents', false)
on conflict (id) do nothing;

drop policy if exists "read patient documents" on storage.objects;
create policy "read patient documents" on storage.objects
  for select using (
    bucket_id = 'patient-documents' and public.category_read_ok('patients')
    and public.can_access_hospital(((storage.foldername(name))[1])::uuid)
  );

drop policy if exists "upload patient documents" on storage.objects;
create policy "upload patient documents" on storage.objects
  for insert with check (
    bucket_id = 'patient-documents' and public.category_write_ok('patients')
    and public.can_access_hospital(((storage.foldername(name))[1])::uuid)
  );

drop policy if exists "delete patient documents" on storage.objects;
create policy "delete patient documents" on storage.objects
  for delete using (bucket_id = 'patient-documents' and public.is_admin());

-- ─────────────────────────── realtime ───────────────────────────
-- Lets index.html subscribe to live changes on hospital_data (see
-- subscribeToRealtime() in the app) so one person's save shows up on
-- everyone else's screen automatically. RLS still applies to what a
-- subscriber actually receives, same as any other query.
do $$
begin
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'hospital_data'
  ) then
    alter publication supabase_realtime add table public.hospital_data;
  end if;
end $$;
